...
# Helper function to add to existing RollingOrigin class
def add_sample_info_method():
    """
    This should be added to the existing RollingOrigin class in timeseries_datamodule.py
    
    def _get_sample_info(self, idx: int) -> Dict[str, int]:
        '''Get detailed information about a sample index.'''
        origin_idx = idx + self.lookback
        start_idx = origin_idx - self.lookback
        end_idx = origin_idx
        target_start = origin_idx + 1
        target_end = origin_idx + self.horizon + 1
...
if __name__ == "__main__":
    print("Improved Time Series Cross-Validation with RollingOrigin Alignment")
    print("=" * 70)
    print("Key improvements:")
    print("1. ✅ CV folds aligned with RollingOrigin dataset boundaries")
    print("2. ✅ Per-fold training matches RollingOrigin data subsets")
    print("3. ✅ Per-origin loss extraction for statistical testing")
    print("4. ✅ Proper train/validation subset alignment")
    print("5. ✅ Support for custom loss functions in CV")
    print()
    print("Note: Add _get_sample_info method to RollingOrigin class for full compatibility")
